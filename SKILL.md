...
├── handlers.py       # BaseHandler + TaskStore
├── retry.py          # RetryHandler（指数退避）
└── utils.py          # insert_text_with_newlines（换行文本注入）
```
...
```python
from browser_scheduler import BaseHandler, HandlerContext, HandlerResult, TaskStore

class MyHandler(BaseHandler):
    def _do_execute(self, ctx: HandlerContext) -> HandlerResult:
        ctx.page.fill("input", ctx.task.prompt_content)
        ctx.page.click("button[type='submit']")
        return HandlerResult(task=ctx.task, status_changed=True)

# 使用
store = TaskStore("tasks.json")
handler = MyHandler(store=store)
result = handler.execute(HandlerContext(task=task, page=page))
```
