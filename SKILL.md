...
- 支持断点续跑（已完成的任务自动跳过）
- 失败任务可重试
- 任务状态自动保存到输出目录下的 `gemini_store.json`

---

### `run` — 生成单个任务

从 prompt 目录中生成指定 ID 的任务（对应 `.md` 文件名，不含后缀）。
...
列出所有任务或查看特定任务的详细信息（状态、输出路径、错误信息）。
...
重试任务目录中所有待处理任务，或仅重试失败的任务。重试时若有 `conversation_url`，会直接打开对话页面；若图片已生成则直接下载。
